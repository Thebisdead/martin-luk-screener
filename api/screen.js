...
async function fetchTickerData(ticker) {
  ...
  try {
    ...
    const realtimeChg1d  = prevClose ? +((realtimePrice - prevClose) / prevClose * 100).toFixed(2) : 0;
    ...
    return { bars, realtimePrice, realtimeChg1d, realtimeVolume, prevClose };
  } catch { return null; }
}
...
// AVWAP anchored at lowest low in lookback window
 ...
 */
function getPullbackPct(bars, n = 20) {
  const recentHigh = Math.max(...bars.slice(-n).map(d => d.h));
  const price      = bars.at(-1).c;
  return +((price - recentHigh) / recentHigh * 100).toFixed(1); // negative = pulled back
}
...
function scoreStock(d) {
  ...
  if (d.adr >= 5)          s +=  5; // enough daily range for good R/R
